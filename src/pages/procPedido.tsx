...
export default function ProcPedido() {
  ...
  React.useEffect(() => {
    ...
    if ((list.length == 0 && user.token )||load) {
      rec();
    }
  }, [list, user,load]);
  ...
  const process = async (pedido: any) =>{
    if(pedido["4status"] == "Enviado"){
        const date = pedido["3data"].split("/")
        ...
        const i = await axios.put(`${URLBase}/pedidos/idp/${pedido["1idPedido"]}`,a,config)
        setLoad(true)
    }
    if(pedido["4status"] == "Recebido"){
        router.push(`cadAbate/${pedido["1idPedido"]}`)
    }
  }

  const cancel = async (pedido: any) =>{
    const date = pedido["3data"].split("/")
    const newDate = date[2]+"-"+date[1]+"-"+date[0]
    const a = {
        dataPedido:newDate,
                    ...
                    return (
                      ...
                      >
                        <p>
                          {element}:{" "}
                          {list.filter((elment)=>{
                            if(elment["4status"] != "Processado" && elment["4status"] != "Cancelado" ){
                                return elment
                            }
                            }).filter((element1) => {
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-2 py-2 font-medium flex flex-col gap-y-1">
                            ...
                            onClick={()=>{
                             ...
                             {element["4status"]== "Enviado" ? <p>Confirmar Recebimento</p>: <p>Cadastrar Abates</p>}
                            </button>
